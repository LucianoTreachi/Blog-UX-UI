export const data = [
  {
    url: "alineacion-de-texto-descubre-la-eleccion-perfecta",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696611356/blog-ux-ui/cover-altura-de-linea_he3sp3.jpg",
    title: "¿Alineación Izquierda o Justificada? Descubre la Elección Perfecta",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          La alineación del texto en una interfaz de usuario desempeña un papel
          fundamental en la legibilidad.
        </p>
        <p>
          Generalmente, en el diseño web se recomienda utilizar la mayor parte
          del tiempo <b>alineación a la izquierda</b>, ya que así estamos
          acostumbrados a leer. Este tipo de alineación permite una lectura más
          fluida y natural, y facilita la comprensión del contenido.
        </p>
        <p>
          ¿Y qué pasa con la alineación justificada? Si bien puede parecer una
          opción atractiva, a menudo crea espacios irregulares entre las
          palabras y desorden en el texto, lo que puede comprometer la
          legibilidad y la apariencia del diseño.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1725888868/blog-ux-ui/alineacion-de-texto-no_u2hxmt.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1725888868/blog-ux-ui/alineacion-de-texto-si_uocn1k.jpg",
  },

  {
    url: "la-fuente-y-su-impacto-en-la-legibilidad",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696612268/blog-ux-ui/cover-tama%C3%B1o-de-fuente_ybkhx5.jpg",
    title: "El tamaño de Fuente y su Impacto en la Legibilidad",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          El tamaño de fuente desempeña un papel esencial en la legibilidad del
          texto en una interfaz de usuario
        </p>
        <p>
          Cuando el texto es demasiado pequeño, los usuarios pueden enfrentar
          dificultades para leerlo, lo que puede resultar en una experiencia
          frustrante. Por otro lado, si el texto es excesivamente grande,
          también puede resultar incómodo, ya que los usuarios tendrán que
          desplazarse constantemente para leer un solo párrafo.
        </p>
        <p>
          Para lograr un equilibrio óptimo, se recomienda utilizar un tamaño
          mínimo de fuente entre 16px a 20px (o su equivalente en rem) para el
          cuerpo del texto.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696938166/blog-ux-ui/tama%C3%B1o-de-fuente-no_ttohkm.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696938167/blog-ux-ui/tama%C3%B1o-de-fuente-si_icfzuq.jpg",
  },

  {
    url: "line-height-la-brecha-entre-una-buena-y-mala-lectura",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696429416/blog-ux-ui/cover-alineacion_rqpqvr.jpg",
    title: "Line-Height: La Brecha entre una Buena y Mala Lectura",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          El line height (altura de línea), se refiere al espacio vertical entre
          las líneas de texto en un párrafo o título. Esta medida puede parecer
          irrelevante, pero tiene un impacto significativo en la legibilidad y
          la comodidad de lectura de un sitio web o una aplicación.
        </p>
        <p>
          Especificar un line-height en los párrafos es una práctica efectiva:
          facilita la lectura y reduce la fatiga ocular.
        </p>
        <p>
          Un line-height entre 1.2 y 1.6 es un buen rango para experimentar y
          encontrar una altura de línea que mejor funcione para un diseño en
          particular.
        </p>
        <p>
          Lograr que los usuarios puedan leer y comprender el contenido de
          manera eficiente es esencial para una experiencia positiva.
        </p>
        <p>
          La legibilidad no solo mejora la usabilidad, sino que también
          contribuye a la percepción general de calidad de tu sitio web o
          aplicación.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696938411/blog-ux-ui/line-height-no_nlgizh.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696938411/blog-ux-ui/line-height-si_ow1znu.jpg",
  },

  {
    url: "pixel-vs-rem-cuando-utilizar-estas-unidades-de-medida",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1713290373/blog-ux-ui/cover-px-rem_ddhcvp.jpg",
    title: "Pixel vs Rem ¿Cuándo utilizar estas unidades de medida?",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          Cuando estamos diseñando una interfaz de usuario, es crucial elegir la
          unidad de medida adecuada para garantizar una experiencia consistente
          y adaptable en diferentes dispositivos y pantallas.
        </p>
        <p>
          <b>La unidad rem</b> es ideal para crear diseños flexibles y
          responsivos que se vean bien en varios tamaños de pantalla y respondan
          a las preferencias del usuario. Es conveniente utilizarla en el tamaño
          de fuente (font-size), márgenes (margin), y relleno interno (padding).
        </p>
        <p>
          <b>La unidad px</b> es absoluta y no responde a cambios en el tamaño
          de fuente raíz ni a las preferencias del usuario. Por lo tanto, sería
          conveniente dejarla reservada para valores de bordes (border), sombras
          (box-shadow, text-shadow) y en situaciones donde se requieran medidas
          fijas.
        </p>
        <p>
          Es crucial entender las fortalezas y limitaciones de cada unidad para
          tomar decisiones informadas y crear interfaces de usuario que sean
          tanto visualmente atractivas como funcionales en cualquier entorno.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1725889499/blog-ux-ui/px-o-rem-no_soxevc.png",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1725889499/blog-ux-ui/px-o-rem-si_acbygr.png",
  },

  {
    url: "La-magia-de-los-parrafos-cortos",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696617298/blog-ux-ui/cover-bloques-de-texto_iqowfl.jpg",
    title: "La Magia de los Párrafos Cortos ¡Divide y Conquistarás!",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          Los usuarios a menudo escanean el contenido en lugar de leerlo de
          principio a fin, y rara vez quieren leer grandes bloques de texto. En
          lugar de esto, es mejor dividir el contenido en párrafos más pequeños
          para que sea más digerible.
        </p>
        <p>
          Al dividir el contenido en párrafos más pequeños, se facilita la
          lectura y los usuarios pueden absorber información de manera más
          eficiente y cómoda, lo que mejora significativamente su experiencia.
        </p>
        <p>
          Cada párrafo puede contener una idea o un tema específico, lo que
          permite que los usuarios se enfoquen en una parte del contenido a la
          vez.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1697108465/blog-ux-ui/bloques-de-texto-no_ssyfjn.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1697108464/blog-ux-ui/bloques-de-texto-si_rstoni.jpg",
  },

  {
    url: "el-ancho-perfecto-de-los-parrafos",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1725889922/blog-ux-ui/cover-parrafo-anchura_nn11nn.jpg",
    title: "El Ancho Perfecto de los Párrafos en Diseño Web",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          En el diseño de interfaces web, la legibilidad es un aspecto crucial
          para garantizar una experiencia de usuario satisfactoria. Uno de los
          factores que influyen significativamente en la legibilidad de un texto
          es el ancho de los párrafos.
        </p>
        <p>
          Las filas de texto demasiado largas pueden resultar abrumadoras para
          los usuarios y dificultar la lectura. Además, un ancho excesivo puede
          hacer que los ojos del lector se cansen más rápido, lo que reduce la
          atención y la comprensión del contenido.
        </p>
        <p>
          Por otro lado, un ancho de línea demasiado estrecho puede fragmentar
          el texto y hacer que la lectura sea tediosa. En resumen, encontrar el
          equilibrio adecuado es fundamental para garantizar una experiencia de
          lectura óptima.
        </p>
        <p>
          El ancho ideal para los párrafos en una interfaz web suele situarse
          entre 500 y 700 píxeles. Este rango proporciona suficiente espacio
          para que el texto se distribuya de manera uniforme sin abrumar al
          usuario. Al limitar el ancho de la línea dentro de este rango, podemos
          mejorar la legibilidad y la comodidad de lectura.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1713469617/blog-ux-ui/parrafo-ancho-no_uvu6oa.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1713469618/blog-ux-ui/parrafo-ancho-si_izagnw.jpg",
  },

  {
    url: "titulos-impactantes-la-llave-para-enganchar-a-tu-audiencia",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696620723/blog-ux-ui/cover-copy-titulos_wayhov.jpg",
    title: "Títulos Impactantes: La Llave para Enganchar a tu Audiencia",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          En lugar de limitarte a describir tus servicios o actividades, opta
          por utilizar frases impactantes que atrapen la atención de los
          visitantes desde el primer momento en que llegan a tu sitio web o
          aplicación.
        </p>
        <p>
          El objetivo es cautivar la atención del visitante y generar un impacto
          instantáneo. Un título impactante puede despertar la curiosidad,
          transmitir la esencia de tu contenido y crear un vínculo emocional con
          el usuario.
        </p>
        <p>
          Además, los títulos efectivos son esenciales para la optimización de
          motores de búsqueda (SEO), ya que ayudan a tu contenido a destacar
          entre la competencia.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696938515/blog-ux-ui/titulos-mas-atractivos-no_pl5puf.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1725890413/blog-ux-ui/titulos-mas-atractivos-si_tsnvyc.jpg",
  },

  {
    url: "efecto-maquina-de-escribir-usarlo-o-evitarlo",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696619710/blog-ux-ui/cover-efecto-maquina-escribir_skzzxv.jpg",
    title: "¿Deberías Usar o Evitar el Efecto Máquina de Escribir?",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          El efecto máquina de escribir en títulos u oraciones es una técnica
          que se utiliza para añadir interactividad y atractivo visual a un
          sitio web o aplicación.
        </p>
        <p>
          Sin embargo, a nivel de experiencia de usuario no es lo mejor. Te
          cuento por qué deberías tener cuidado con este efecto: <br />
          <br />
          <b>• Tiempo de espera:</b> el usuario tiene que esperar a que se
          complete la oración antes de poder leerla completamente. <br />
          <br />
          <b>• Dificultades de lectura:</b> si el usuario tiene problemas de
          visión o dislexia, el efecto máquina de escribir puede dificultar aún
          más la lectura.
          <br />
          <br />
          <b>• Comprensión rápida:</b> el efecto máquina de escribir puede
          afectar la capacidad del usuario para obtener una comprensión rápida
          de lo que ofrece el sitio web.
          <br />
          <br />
          <b>• Confusión y distracción:</b> dependiendo de cómo se implemente el
          efecto máquina de escribir, puede generar saltos de línea, movimiento
          de textos cercanos, o movimiento en la pantalla.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696939229/blog-ux-ui/maquina-de-escribir-no_cogyir.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696939229/blog-ux-ui/maquina-de-escribir-si_msg49q.jpg",
  },

  {
    url: "texto-azul-en-parrafos",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696592938/blog-ux-ui/cover-texto-azul_dmzzjw.jpg",
    title: "Texto Azul en Párrafos ¿Cuándo es Apropiado?",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          El color azul se ha convertido en una convención ampliamente
          reconocida para los hipervínculos en la web, y su uso coherente mejora
          la navegación y la experiencia del usuario.
        </p>
        <p>
          Mantener el texto azul reservado exclusivamente para los enlaces
          permite a los usuarios identificar rápidamente las áreas interactivas
          en tu sitio web o aplicación. Esto facilita la comprensión y la
          interacción, reduciendo la confusión y mejorando la usabilidad de tu
          diseño.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696939354/blog-ux-ui/texto-azul-no_nbb5tq.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696939354/blog-ux-ui/texto-azul-si_qz0qqr.jpg",
  },

  {
    url: "superposicion-de-imagenes-y-texto",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696620951/blog-ux-ui/cover-superposicion_warj9g.jpg",
    title: "Superposición de Imágenes y Texto ¡Crea Armonía Visual!",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          La superposición de imagen y texto puede ser un desafío si no se
          eligen las imágenes adecuadas.
        </p>
        <p>
          La combinación de imágenes y texto permite a los diseñadores
          transmitir mensajes de manera más rica y efectiva. Esta práctica es
          esencial para atraer y retener la atención del usuario.
        </p>
        <p>
          El mayor desafío al superponer texto sobre una imagen es garantizar
          que el texto sea legible y que la imagen no se vea obstruida o
          distorsionada.
        </p>
        <p>
          Para abordar este desafío, es crucial seleccionar imágenes que tengan
          áreas limpias o espacios donde el texto pueda encajar sin problemas.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696621740/blog-ux-ui/superposicion-no_mnjt4h.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696621741/blog-ux-ui/superposicion-si_kttuja.jpg",
  },

  {
    url: "mejorando-la-estetica-y-legibilidad-con-degradados-lineales",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696429416/blog-ux-ui/cover-degradado_uhk10d.jpg",
    title: "Mejorando la Estética y Legibilidad con Degradados Lineales",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          Los degradados lineales son una herramienta versátil que puede
          transformar de manera significativa la apariencia de tu diseño,
          aumentar la legibilidad y mejorar la experiencia del usuario.
        </p>
        <p>
          Considera utilizarlos en fondos o tarjetas para agregar un toque
          visualmente atractivo a tu interfaz. Experimenta con diferentes
          combinaciones de colores y ángulos para lograr efectos visuales que no
          solo atraigan la atención del usuario, sino que también hagan que tu
          diseño sea más agradable a la vista.
        </p>
        <p>
          Los degradados lineales pueden proporcionar un fondo suave y atractivo
          que permite que el texto destaque de manera efectiva.
        </p>
        <p>
          Este recurso también puede añadir una capa de sofisticación y belleza
          a tu diseño, mejorando la impresión general que tiene el usuario de tu
          interfaz.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1725894816/blog-ux-ui/degradado-lineal-no_w8ar6w.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1725894816/blog-ux-ui/degradado-lineal-si_iimvcy.jpg",
  },

  {
    url: "flechas-de-regreso",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696601107/blog-ux-ui/cover-icono-volver_c2a6id.jpg",
    title: "Flechas de Regreso: Más Allá de la Estética",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          Cuando diseñamos interfaces de usuario, la claridad visual es
          fundamental para garantizar una experiencia de usuario óptima. Una
          forma sencilla de aumentar la comprensión de un elemento en tu diseño
          es añadir un icono de flecha apuntando hacia la izquierda,
          especialmente cuando se trata de una acción como &quot;Volver al
          sitio&quot;, &quot;Volver a inicio&quot;, &quot;Volver atrás&quot;.
        </p>
        <p>
          La razón detrás de esto es que la mayoría de las personas asocian
          naturalmente una flecha hacia la izquierda con la acción de retroceder
          o volver atrás. Al agregar esta indicación visual, estás comunicando
          de manera más efectiva la función del elemento, lo que facilita que
          los usuarios comprendan y utilicen tu aplicación o sitio web de manera
          más fluida.
        </p>
        <p>
          Asegúrate de que el icono de flecha sea claramente visible y fácil de
          identificar. Puedes usar diferentes tamaños o colores para destacarlo
          según el contexto de tu diseño, pero la consistencia en el uso de este
          tipo de iconos en toda tu interfaz ayudará a crear una experiencia
          cohesiva y amigable para el usuario.
        </p>
        <p>
          Recuerda que el diseño de una interfaz debe ser intuitivo y accesible,
          y pequeños detalles como esta flecha pueden marcar una gran diferencia
          en la percepción y usabilidad de tu producto. No subestimes el poder
          de la claridad visual al diseñar tus elementos de navegación.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696939725/blog-ux-ui/icono-volver-no_x9g56n.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696939726/blog-ux-ui/icono-volver-si_qjblkf.jpg",
  },

  {
    url: "ley-de-von-restorff",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696429417/blog-ux-ui/cover-von-restorff_zumfkg.jpg",
    title: "Ley de Von Restorff ¡Destaca lo diferente!",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          ¿Qué nos enseña esta ley? Cuando presentamos una lista de elementos de
          una misma categoría junto con uno que difiere en algún aspecto (ya sea
          en tamaño, forma, color, etc.), este último será más memorable y
          captará mayor atención.
        </p>
        <p>
          Por lo tanto, utiliza este recurso en tu interfaz de usuario cuando
          desees enfatizar una funcionalidad por encima del resto o reforzar una
          acción específica.
        </p>
        <p>Jugar con esta ley es jugar con la atención del usuario.</p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1725890855/blog-ux-ui/von-restorff-no_uiv2rp.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1725890855/blog-ux-ui/von-restorff-si_t7lknk.jpg",
  },

  {
    url: "ley-de-hick",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696429417/blog-ux-ui/cover-hick_hgeot3.jpg",
    title: "Ley de Hick ¡Simplifica la Experiencia de Usuario!",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          ¿Qué nos enseña esta ley? A medida que aumentan las opciones y la
          complejidad en un diseño, el usuario puede experimentar dificultades
          para comprender qué acciones puede realizar y, por ende, tomará
          decisiones más lentas o incluso no tomará ninguna decisión.
        </p>
        <p>
          Es interesante utilizar este recurso en formularios de alta, registro
          o compra, navegación, menú dropdown, opciones múltiples, sección de
          features, sección Home.
        </p>
        <p>
          Mantener la interfaz simple, limpia, intuitiva y fácil de usar,
          mejorará significativamente la usabilidad y la satisfacción del
          usuario.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1725894308/blog-ux-ui/hick-no_kuk9cm.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1725894308/blog-ux-ui/hick-si_qrlix4.jpg",
  },

  {
    url: "ley-de-fitts",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696947267/blog-ux-ui/cover-fitts_m88zmx.jpg",
    title: "Ley de Fitts ¡El Arte de Hacer Clic sin Esfuerzo!",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          ¿Qué nos enseña esta ley? El tamaño y la distancia de un objetivo
          (ejemplo: un botón, un enlace) influyen directamente en el tiempo
          necesario para alcanzarlo.
        </p>
        <p>
          Por lo tanto, utiliza este recurso para mantener los botones o enlaces
          principales lo suficientemente grandes, y ubicados cerca del área del
          pulgar del usuario.
        </p>
        <p>
          Al optimizar el tamaño y la colocación de los elementos interactivos
          (como botones y enlaces), se reducirán los esfuerzos requeridos por
          los usuarios para alcanzar y seleccionar estos objetivos.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696940374/blog-ux-ui/fitts-no_bimidh.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696940374/blog-ux-ui/fitts-si_k5bnot.jpg",
  },

  {
    url: "ley-de-jakob",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1713463976/blog-ux-ui/cover-jacob_zudksz.jpg",
    title: "Ley de Jakob ¡Una Regla de Oro!",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          ¿Qué nos enseña esta ley? Los usuarios pasan la mayor parte del tiempo
          en otros sitios web y aplicaciones, lo que los hace inclinarse hacia
          productos que ya les resultan familiares y funcionales. Cuando los
          usuarios ya están familiarizados con algo y esto se modifica, lo
          primero que sienten es rechazo o frustración.
        </p>
        <p>
          Por lo tanto, a la hora de crear un sitio web o aplicación, es
          recomendable seguir los diseños y patrones ya conocidos por los
          usuarios.
        </p>
        <p>
          Si aplicamos la ley de Jakob Nielsen, le estamos ahorrando a nuestros
          usuarios un mayor esfuerzo cognitivo.
        </p>
        <p>
          A continuación, podemos apreciar cómo en una página de ecommerce se
          prioriza mantener un diseño familiar y reconocible para el usuario, lo
          que reduce su carga cognitiva. Por ejemplo, el logo suele ubicarse en
          la parte superior izquierda, mientras que los iconos de búsqueda,
          acceso a la cuenta y carrito suelen estar en la parte superior
          derecha.
        </p>
        <p>
          En cuanto a la página de detalle del producto, es común encontrar la
          imagen del producto a la izquierda, mientras que a la derecha se
          presenta información crucial como el título, el precio, las opciones
          de talla, un contador y un botón para &quot;Agregar al carrito&quot;,
          entre otros elementos relevantes.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1713462308/blog-ux-ui/ley-jacob-no_uomfdl.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1713462309/blog-ux-ui/ley-jacob-si_nkuhna.jpg",
  },

  {
    url: "chatbots-y-mensajes-automaticos-ayuda-o-molestia",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696592937/blog-ux-ui/cover-chatbots_vk4b7l.jpg",
    title: "Chatbots y Mensajes Automáticos ¿Ayuda o Molestia?",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          Interrumpir a un usuario tan pronto como ingresa a un sitio web o
          aplicación puede ser perjudicial para su experiencia. Abrir un cuadro
          de diálogo sin que el usuario haya tomado ninguna acción previa puede
          resultar molesto y contraproducente
        </p>
        <p>
          Hoy en día, muchos sitios web bombardean a los visitantes con chats o
          mensajes automáticos del tipo &quot;¿En qué puedo ayudarte?&quot;, o
          &quot;Soy Luisa, tu asistente virtual&quot;. Sin embargo, es
          importante recordar que la libertad del usuario es fundamental. En
          lugar de imponer una conversación, es preferible permitir que el
          usuario inicie el diálogo o chat cuando lo necesite y esté listo para
          hacerlo.
        </p>
        <p>
          Esta práctica respeta la autonomía del usuario y garantiza que la
          interacción sea genuina y valiosa para ambas partes.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696939854/blog-ux-ui/chatbots-no_w1rj5t.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696939854/blog-ux-ui/chatbots-si_y1c8xw.jpg",
  },

  {
    url: "la-importancia-del-modal-de-confirmacion",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1713185050/blog-ux-ui/cover-eliminar-items_iuv2ka.jpg",
    title: "La Importancia del Modal de Confirmación ¡Evita errores!",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          Es importante tener en cuenta que algunas acciones como &quot;eliminar
          producto&quot;, &quot;vaciar carrito&quot;, &quot;cancelar
          compra&quot;, &quot;eliminar cuenta&quot;, &quot;borrar archivo&quot;,
          etc., pueden tener consecuencias graves y permanentes.
        </p>
        <p>
          Por lo tanto, cuando el usuario presione sobre un botón que lleve a
          una acción de este tipo, no permitas que se ejecute inmediatamente
          dicha acción, en su lugar agrega un modal de confirmación con 2
          botones: un botón primario para confirmar y un botón secundario para
          cancelar.
        </p>
        <p>
          Al agregar un modal de confirmación, estás proporcionando una
          oportunidad para que los usuarios reconsideren su acción y eviten
          cualquier error involuntario.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696939972/blog-ux-ui/eliminar-items-no_ivmwe4.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696939973/blog-ux-ui/eliminar-items-si_mv2ybn.jpg",
  },

  {
    url: "boton-primario-boton-secundario",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696937894/blog-ux-ui/cover-botones_vlbtfy.jpg",
    title: "Botón Primario y Botón Secundario: ¿Cuál es la diferencia?",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          Los botones primarios y secundarios son una parte fundamental del
          diseño de páginas web y aplicaciones, ya que su correcta distinción y
          diseño adecuado puede influir significativamente en la experiencia del
          usuario.
        </p>
        <p>
          El botón primario es aquel que tiene mayor peso e importancia, suele
          representar la acción principal que se espera que el usuario realice
          en la página o aplicación, y suele tener un color más llamativo.
        </p>
        <p>
          El botón secundario es aquel que tiene menor peso, suele representar
          la opción de cancelar o posponer una acción, y tiene un color menos
          llamativo.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1725892650/blog-ux-ui/botones-no_obgwi0.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1725892650/blog-ux-ui/botones-si_qoqhx0.jpg",
  },

  {
    url: "botones-tipo-radio-en-formularios",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696850058/blog-ux-ui/cover-botones-radio_vqpxhy.jpg",
    title: "Botones Tipo Radio: Simplificando Formularios",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          Cuando estés diseñando un formulario con pocas opciones, considera
          utilizar botones tipo radio en lugar de listas desplegables.
        </p>
        <p>
          Los botones tipo radio ofrecen una selección clara y rápida para los
          usuarios, reduciendo la complejidad y agilizando la toma de
          decisiones.
        </p>
        <p>
          Esto mejora la experiencia del usuario al hacer que el proceso sea más
          eficiente y fácil de entender.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1697108604/blog-ux-ui/botones-radio-no_ig3oi4.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1697108604/blog-ux-ui/botones-radio-si_wx8ygu.jpg",
  },

  {
    url: "iconos-en-mensajes-de-error",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696516463/blog-ux-ui/cover-icono-mensajes-error_n2mnrj.jpg",
    title: "Iconos en Mensajes de Error: El Arte de la Comunicación Visual",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          Los mensajes de error son esenciales para guiar a los usuarios cuando
          algo sale mal, pero su efectividad puede aumentar significativamente
          cuando se les añade un toque visual.
        </p>
        <p>
          Considera incorporar íconos relevantes junto a los mensajes de error
          para comunicar de manera más efectiva la naturaleza del problema. Los
          íconos aportan un elemento visual que refuerza el mensaje.
        </p>
        <p>
          Esta práctica no solo mejora la comprensión, sino que también puede
          hacer que la experiencia sea más agradable y memorable para los
          usuarios.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696940752/blog-ux-ui/icono-mensaje-error-no_nzl7sz.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696940753/blog-ux-ui/icono-mensaje-error-si_wu3ve4.jpg",
  },

  {
    url: "mensaje-de-error-en-formularios",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696593772/blog-ux-ui/cover-error-input_pqleh7.jpg",
    title: "Mensajes de Error en Formularios: Por qué la Ubicación Importa",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          En lugar de agrupar todos los mensajes de error en la parte superior o
          inferior del formulario, una práctica más efectiva y amigable para el
          usuario es colocar los mensajes de error directamente debajo de los
          campos correspondientes.
        </p>
        <p>
          Cuando un mensaje de error se muestra junto al campo donde se produjo
          el error, los usuarios pueden identificar instantáneamente cuál es el
          problema y cómo corregirlo.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696941049/blog-ux-ui/mensajes-error-input-no_lfzrp9.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696941050/blog-ux-ui/mensajes-error-input-si_yjfpay.jpg",
  },

  {
    url: "simplificando-formularios-extensos",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696603881/blog-ux-ui/cover-formulario-largo_i4vxka.jpg",
    title: "Simplificando Formularios Extensos ¡Agrupa campos!",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          Cuando los usuarios se enfrentan a formularios extensos, la sobrecarga
          de información puede ser abrumadora.
        </p>
        <p>
          Una estrategia para reducir la carga cognitiva puede ser agrupar
          campos relacionados de manera efectiva.
        </p>
        <p>
          Cuando agrupes campos, asegúrate de utilizar etiquetas claras y
          descriptivas para cada sección. Además, considera el uso de colores o
          separadores visuales sutiles para destacar las divisiones entre las
          secciones.
        </p>
        <p>
          Recuerda que el objetivo es simplificar la experiencia del usuario y
          hacer que completar el formulario sea una tarea más fácil y agradable.
        </p>
        <p>
          Al reducir la carga cognitiva, estarás más cerca de lograr que los
          usuarios completen con éxito el proceso que tienes para ellos.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696941094/blog-ux-ui/formularios-extensos-no_b0zpwu.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696941094/blog-ux-ui/formularios-extensos-si_xkazsf.jpg",
  },

  {
    url: "el-secreto-de-generar-confianza-en-el-usuario",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696526313/blog-ux-ui/cover-mensaje-cta_m8nhr5.jpg",
    title: "El Secreto de generar Confianza en el Usuario",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          En algunas situaciones, es bueno proporcionar algo de información
          extra sobre lo que sucederá luego de una acción. De esta forma,
          garantizamos seguridad, generamos confianza, y mejoramos la
          experiencia de usuario.
        </p>
        <p>
          Por ejemplo, en situaciones donde el usuario se suscriba a un plan o
          adquiera un producto digital que requiera el ingreso de los datos de
          su tarjeta de crédito o débito, considera agregar un mensaje
          informativo debajo del botón de llamada a la acción (CTA).
        </p>
        <p>
          Esta información adicional brinda claridad y confianza al usuario
          antes de tomar una decisión importante.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1696941134/blog-ux-ui/mensaje-informativo-cta-no_qvoigo.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1697108758/blog-ux-ui/mensaje-informativo-cta-si_rxbbga.jpg",
  },

  {
    url: "como-diseñar-un-hero-atractivo-que-llame-la-atención",
    cover:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1713532354/blog-ux-ui/cover-home_bhoh3o.jpg",
    title: "¿Cómo diseñar un Hero atractivo que llame la atención?",
    time: "Lectura: 2 min",
    description: (
      <>
        <p>
          Cuando un visitante entra en un sitio web, su primera impresión puede
          determinar si se queda y explora más o si abandona la página. Para
          asegurarnos de captar su atención de manera efectiva, es crucial tener
          un buen titulo, un buen subtítulo, un botón de llamada a la acción,
          prueba social, y una imagen atractiva.
        </p>
        <p>
          • El titulo principal debe comunicar claramente la propuesta de valor
          de tu empresa o marca de manera concisa. El titulo comunica beneficios
          y ventajas.
        </p>
        <p>
          • El subtítulo complementa al título principal, ofreciendo una
          explicación más detallada de cómo se logrará la propuesta de valor
          mencionada. Debe ser relevante y persuasivo, motivando al visitante a
          querer aprender más sobre lo que ofreces.
        </p>
        <p>
          • El botón de llamada a la acción indica al visitante cuál es el
          siguiente paso que debe tomar para obtener más información o realizar
          una acción específica. Debe ser claro, relevante y destacarse
          visualmente para incentivar la interacción.
        </p>
        <p>
          • La prueba social es un elemento poderoso que puede reforzar la
          credibilidad y confianza en tu marca. La inclusión de pruebas sociales
          puede ayudar a convencer al visitante de que tu producto o servicio es
          digno de su atención.
        </p>
        <p>
          • La imagen debe ser visualmente impactante y relevante para tu marca
          o producto. Debe capturar la atención del visitante y transmitir la
          esencia de lo que ofreces de manera rápida y efectiva.
        </p>
      </>
    ),
    image1:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1713538751/blog-ux-ui/seccion-home-no_ktjtjs.jpg",
    image2:
      "https://res.cloudinary.com/dwqk2mkgh/image/upload/f_auto,q_auto/v1713538751/blog-ux-ui/seccion-home-si_likkfi.jpg",
  },
];
